...
class OrderManager:
    ...
    def __init__(self, broker, capital_allocator):
        ...
        try:
            cfg = get_config()
            # Only attempt database connection if in live mode or if DB config is provided
            # This prevents backtests from stalling on connection retries.
            if getattr(broker, 'mode', 'live') == 'live' or os.getenv('ENABLE_DB_PERSISTENCE') == 'true':
                ...
                logging.info("OrderManager: Skipping database persistence (running in paper/backtest mode).")
